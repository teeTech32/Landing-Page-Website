(function(){async function checkAdBlock(){let blocked;async function request(){try{return fetch(new Request("https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js",{method:"HEAD",mode:"no-cors"})).then(function(){blocked=false;return blocked;}).catch(function(e){blocked=true;return blocked;});}catch(error){blocked=true;return blocked;}}
return blocked!==undefined?blocked:await request();}
function replaceAds(){if(!adsOptions){return;}
adsOptions.forEach((option)=>{let ads=document.querySelectorAll(option.selector);if(ads){for(let i=0;i<ads.length;i++){ads[i].innerHTML=option.code;}}});}
addEventListener("DOMContentLoaded",async function(){const detected=await checkAdBlock();if(detected){replaceAds();}});})();
...
const setTheme=(theme)=>(root.dataset.theme=theme);const getTheme=()=>root.dataset.theme;switcher.addEventListener("click",toggleTheme);function toggleTheme(){let theme=getTheme()==="dark"?"light":"dark";setTheme(theme);localStorage.setItem("theme",theme);}})();
(function(){let siteNav=document.querySelector(".site-menu");if(!siteNav){return;}
function closeSubmenuExcept(submenu){const submenuList=siteNav.querySelectorAll(".sub-menu");if(submenuList){for(let i=0;i<submenuList.length;i++){if(submenuList[i]!=submenu){submenuList[i].classList.remove("show");}}}}
const parentMenuItems=siteNav.querySelectorAll(".menu-item-has-children");if(parentMenuItems){for(let i=0;i<parentMenuItems.length;i++){parentMenuItems[i].addEventListener("click",function(){const submenu=this.querySelector(".sub-menu");if(submenu){submenu.classList.toggle("show");}
closeSubmenuExcept(submenu);});}}
function closeSubmenu(e){let isClickInside=siteNav.contains(e.target);if(!isClickInside&&siteNav.querySelector(".sub-menu.show")){siteNav.querySelector(".sub-menu.show").classList.remove("show");}}
